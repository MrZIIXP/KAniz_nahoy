...
function updateDisplay() {
	...
	document.getElementById(
		'current-bet'
	).textContent = `Ставка: ${currentBet.toFixed(2)}$`
...
function spinSlot() {
	...
	document.getElementById('slot-grid').innerHTML = result
		.map(s => `<div>${s}</div>`)
		.join('')
	...
	for (let i = 1; i < result.length; i++) {
		if (result[i] === result[i - 1]) {
			currentStreak++
			if (currentStreak > maxInRow) {
				maxInRow = currentStreak
			}
		} else {
			currentStreak = 1
		}
	...
	// Если все 6 совпадают
	if (maxInRow === 6) {
		multiplier = 10
	} else if (maxInRow >= 3) {
		const symbol = result.find((s, i, arr) => arr[i] === arr[i + 1] && arr[i + 1] === arr[i + 2])
		if (symbol === '7') multiplier = 7
		else if (symbol === '💎') multiplier = 3
		else if (symbol === '🍒') multiplier = 2
		else if (symbol === '🍋') multiplier = 1.5
	}
...
function spinRoulette() {
	...
	document.getElementById(
		'roulette-result'
	).textContent = `Результат: ${resultNumber} ${resultColor}`
